...
class UserRepository implements UserRepositoryInterface
{
    ...
    {
        $this->db->execute('INSERT INTO user_achievements (user_id, achievement_id, display_order) VALUES (:uid, :aid, :display) ON DUPLICATE KEY UPDATE display_order = :display', ['uid' => $userId, 'aid' => $achievementId, 'display' => $displayOrder]);
    }

    public function removeAchievement(int $userId, int $achievementId): void
    {
        $this->db->execute('DELETE FROM user_achievements WHERE user_id = :uid AND achievement_id = :aid', ['uid' => $userId, 'aid' => $achievementId]);
    }
}
