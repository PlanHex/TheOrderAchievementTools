...
class UserRepository implements UserRepositoryInterface
{
    ...
    public function addAchievement(int $userId, int $achievementId, int $displayOrder = 0): void
    {
        ...
        $_SESSION[self::SESSION_UA_KEY][$userId][(int)$achievementId] = (int)$displayOrder;
    }

    public function removeAchievement(int $userId, int $achievementId): void
    {
        if (isset($_SESSION[self::SESSION_UA_KEY][$userId][(int)$achievementId])) {
            unset($_SESSION[self::SESSION_UA_KEY][$userId][(int)$achievementId]);
        }
    }
