...
class AchievementRepository implements AchievementRepositoryInterface
{
    ...
    {
        if ($categoryId === null) {
            // Order by category display_order, then achievement display_order
            $rows = $this->db->fetchAll('SELECT a.* FROM achievements a JOIN categories c ON a.category_id = c.id ORDER BY c.display_order ASC, a.display_order ASC, a.id ASC');
        } else {
